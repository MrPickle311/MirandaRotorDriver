...
HAL_StatusTypeDef ROTOR_Reset() {
  ...
  return HAL_I2C_Master_Transmit(rotor_i2c, rotor_write_addr, data,
                                 sizeof(data) / sizeof(uint8_t), max_request_delay);
}
...
HAL_StatusTypeDef ROTOR_GetCalibrationState(enum ROTOR_CalibrationStatus* calibration_state)
{
	...
	return HAL_I2C_Mem_Read(rotor_i2c, rotor_read_addr, command_byte, 1, calibration_state, 1, max_request_delay);
}
...
HAL_StatusTypeDef ROTOR_GetMovementState(enum ROTOR_MovementStatus* movement_status)
{
	...
	return HAL_I2C_Mem_Read(rotor_i2c, rotor_read_addr, command_byte, 1, movement_status, 1, max_request_delay);
}
...
HAL_StatusTypeDef ROTOR_GetCurrentLocation(uint16_t* current_position)
{
	...
	return HAL_I2C_Mem_Read(rotor_i2c, rotor_read_addr, command_byte, 1, (uint8_t*)current_position, 2, max_request_delay);
}
